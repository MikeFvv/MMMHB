...
@interface RongYunManager()
@end

@implementation RongYunManager


+ (RongYunManager *)shareInstance{
    static RongYunManager *instance = nil;
...
- (instancetype)init{
    ...
    if (self) {
        ...
        [RCIM sharedRCIM].userInfoDataSource = self;//[RongYunManager shareInstance];
        [RCIM sharedRCIM].groupInfoDataSource = self;//[RongYunManager shareInstance];
        [RCIM sharedRCIM].receiveMessageDelegate = self;//[RongYunManager shareInstance];
...
#pragma mark RCIMConnectionStatusDelegateGHH
...
- (void)setMode:(NSInteger)mode{
    [[RCIM sharedRCIM] initWithAppKey:(mode==RongYunModel_test)?RyTestKey:RyKey];
}
...
//    }else
    {
        ...
        [FUNCTION_MANAGER updateChat:gId number:number lastMessage:text lastTime:[NSString stringWithFormat:@"%lld",message.receivedTime]];
    }
}
...
+ (void)initWithMode:(RongYunModel)mode{
    [[RongYunManager shareInstance] setMode:mode];
}
...
- (void)startConnect{
    ...
    [[RCIM sharedRCIM] connectWithToken:APP_MODEL.rongYunToken success:^(NSString *userId) {
        RCUserInfo *user = [[RCUserInfo alloc]initWithUserId:userId name:APP_MODEL.user.userNick portrait:APP_MODEL.user.userAvatar];
