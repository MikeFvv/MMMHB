...
-(void)getListWithPacketId:(NSString *)packetId success:(void (^)(NSDictionary *))success
    ...
    [NET_REQUEST_MANAGER requestRedPacketDetailWithId:packetId success:^(id object) {
        [self handleGetListSuccess:object];
        success(object);
    } fail:^(id object) {
        failue(object);
    }];
...
-(void)handleGetListSuccess:(NSDictionary *)dic{
    NSDictionary *data = [dic objectForKey:@"data"];
    if (data != NULL) {
        NSDictionary *detail = data[@"detail"];
        self.user = detail;
        [self.dataList removeAllObjects];        
        NSArray *list = [data objectForKey:@"skRedbonusGrabModels"];
        self.maxMoney = 0.00;
        for (id obj in list) {
            CDTableModel *model = [CDTableModel new];
            model.obj = obj;
            model.className = @"EnvelopTableViewCell";
            CGFloat money = [[obj objectForKey:@"redbgMoney"] floatValue];
