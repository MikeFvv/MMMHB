...
- (void)normalAddButtonTapped:(id *)sender event:(id)event
{
    ...
    if (indexPath != nil)
    {
        FYContacts *newcontact  = [[_listContent objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
        //第一步注册通知
...
- (void)searchAddButtonTapped:(id *)sender event:(id)event
{
    ...
    if (indexPath != nil)
    {
        FYContacts *newcontact  = [_filteredListContent objectAtIndex:indexPath.row];
        //第一步注册通知
