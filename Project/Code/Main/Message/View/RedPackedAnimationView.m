...
@interface RedPackedAnimationView()<CAAnimationDelegate>
...
@implementation RedPackedAnimationView
...
- (void)initSubviews {
    ...
    CGFloat h = (CDScreenWidth-marginWidth)/0.8125;
...
- (void)updateView:(id)obj response:(id)response {
    ...
    if (status == 1) {  // 正常
        if ([userId isEqualToString:[AppModel shareInstance].user.userId]) {  // 自己
